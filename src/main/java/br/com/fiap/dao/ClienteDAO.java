...
public class ClienteDAO extends Repository {
    ...
    public ArrayList<ClienteTO> findAll() {
        ...
        String sql = "select * from aaa_clientes order by codigo";
    ...
    public ClienteTO findByCodigo(Long codigo) {
        ClienteTO cliente = new ClienteTO();
        String sql = "select * from aaa_clientes where codigo = ?";
        try(PreparedStatement ps = getConnection().prepareStatement(sql)) {
            ps.setLong(1, codigo);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    public ClienteTO save(ClienteTO cliente) {
        String sql = "insert into aaa_clientes (nome, cpf, email, data_de_nascimento) values(?, ?, ?, ?)";
        try(PreparedStatement ps = getConnection().prepareStatement(sql)) {
    ...
    public boolean delete(Long codigo) {
        String sql = "delete from aaa_clientes where codigo = ?";
        try(PreparedStatement ps = getConnection().prepareStatement(sql)) {
            ps.setLong(1, codigo);
    ...
    public ClienteTO update(ClienteTO cliente) {
        String sql = "update aaa_clientes set nome=?, cpf=?, email=?, data_de_nascimento=? where codigo=?";
        try(PreparedStatement ps = getConnection().prepareStatement(sql)) {
            ps.setString(1, cliente.getNome());
            ps.setString(2, cliente.getCpf());
            ps.setString(3, cliente.getEmail());
